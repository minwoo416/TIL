...
* `option`태그 작성 시 value속성은 필수가 아닌 사용자가 선택되는 데이터에만 사용한다.<!-- ex) 통신사,kt,skt,lg,알뜰폰을 선택하는 리스트에서 `통신사` 부분은 value사용안해도 O -->
...
* `<inbut id="a"><label for="a">선택글자</label>`
...
* `flex, flot` 등 포함 위치가 잡혀있는 요소에서 상/하/좌/우로 살짝 이동을 할때는 `position:relative;`
